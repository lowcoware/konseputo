...
def _render_pre_research_warning(report: schema.Report) -> list[str]:
    ...
    return [
        "## Pre-Research Status",
        "",
        "  Step 0.55 pre-research was skipped. The engine ran with keyword search only.",
        "",
...
def _render_degraded_run_warning(report: schema.Report) -> list[str]:
    ...
    return [
        ...
        "",
        f"  This run was called BARE on a named-entity topic (\"{report.topic}\"):",
...
def _top_voices_footer_line(report: schema.Report) -> str | None:
    """Return the  Top voices line or None if no meaningful voices exist.
    ...
    return f" Top voices: {' │ '.join(parts)}"
