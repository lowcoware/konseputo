...
A two-dot range compares two **commits**. An autonomous Konseputo-goal run frequently leaves its
...
namespaced artifact dir, e.g. `.konseputo-goal/add-dark-mode-Ab3Kx9`), so the `/goal` session invokes
...
The transcript markers (`KONSEPUTOGOAL_PHASE_VERIFY` cleanliness section, `AUDIT_VERIFY`
