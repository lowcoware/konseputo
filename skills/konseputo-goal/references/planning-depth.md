...
If you can't name three plausible risks, the task is either too small for Konseputo-goal or you haven't thought hard enough.

**Why this file never asks "how many days will this take":** the planning
fallacy (Kahneman & Tversky) is why calendar-time estimates miss so
reliably — people imagine the best-case path and ignore what similar past
work actually took, a bias robust across experience levels that persists
even in people who know about the bias. Software estimates miss by a wide
margin often enough that "estimate, then blow past it" is closer to the
norm than the exception. The one method shown to actually help —
reference-class forecasting (estimate from how long similar past work
took, not from imagining this specific task going smoothly) — is exactly
what phase-count derivation and risk-listing do here instead of a duration
guess: ground the plan in concrete precedent (past pitfalls, real risks
named) rather than an optimistic number that will not survive contact with
the work.
[Instil: the planning fallacy and how it biases estimation](https://instil.co/blog/planning-fallacy/)
