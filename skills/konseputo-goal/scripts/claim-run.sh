#!/usr/bin/env bash
# claim-run.sh — atomically claim a unique per-run artifact directory under .konseputo-goal/.
#
# Why this exists
# ---------------
# Two `/konseputo-goal` runs started in the same working tree both defaulted to a flat
# `.konseputo-goal/` directory and clobbered each other's STATE.md / ROADMAP.md / phases/.
...
#                                          e.g.  .konseputo-goal/add-dark-mode-Ab3Kx9
#
# Nothing but the path is printed, so callers can capture it directly:
#   KONSEPUTOGOAL_ROOT="$(bash claim-run.sh "$ARGUMENTS")"
#
# Honours $KONSEPUTOGOAL_BASE (default ".konseputo-goal") as the parent directory.
