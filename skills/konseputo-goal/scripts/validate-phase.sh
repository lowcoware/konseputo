...
check_marker() {
  ...
  if ! grep -q "$marker" "$f"; then
    echo "FAIL $f: missing $label ($marker)" >&2
    errors=$((errors + 1))
  fi
}
...
check_section() {
  ...
  if ! grep -qi "^## $heading\|^\*\*$heading" "$f"; then
    echo "FAIL $f: missing section: $heading" >&2
    errors=$((errors + 1))
  fi
}

# Required markers
check_marker "KONSEPUTOGOAL_PHASE_START" "phase-start marker"
...
if [[ "$crits" -lt 3 ]]; then
  echo "WARN $f: only $crits bullet lines — acceptance criteria look thin" >&2
fi
