...
// dna-scaffold.mjs — generate a design-dna.json skeleton, best-effort pre-filled from recon-site.mjs output.
// Usage:
//   node scripts/dna-scaffold.mjs --out <design-dna.json> [--recon <label-recon.json>] [--name <site name>]
// Output:
//   <out>  full DNA skeleton; with --recon, fonts/color candidates/framework-effect
//          signals are pre-filled, everything else stays "" for manual analysis.
// Discipline: only carry signals the recon ACTUALLY captured, never invent. Color
//   values whose role (primary/accent) is unclear go into _recon_signals for manual assignment.
...
function usage() {
  console.log(`dna-scaffold.mjs — generate a design-dna.json skeleton, best-effort pre-filled

  node scripts/dna-scaffold.mjs --out <design-dna.json> [--recon <label-recon.json>] [--name <site name>]

Only for visual-clone / rebrand modes. The faithful-clone branch does not use DNA
(real source is the truth). Schema and field meanings: references/design-dna.md.`);
}
...
try {
  ...
  if (args.recon) {
    try {
      ...
      console.warn(`WARN could not read recon (${e.message}); writing empty scaffold only.`);
    }
  }
  const outPath = path.resolve(args.out);
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(outPath, `${JSON.stringify(dna, null, 2)}\n`);
  console.log(`design-dna scaffold written: ${outPath}`);
